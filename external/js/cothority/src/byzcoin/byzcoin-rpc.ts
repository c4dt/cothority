...
export default class ByzCoinRPC implements ICounterUpdater {
    ...
    async instanceObservable(id: InstanceID): Promise<BehaviorSubject<Proof>> {
        ...
        if (this.updateInstances === undefined) {
            // For every new block, send the latest know version of all instances, and then
            ...
            this.updateInstances.pipe(throttleTime(1000))
                .subscribe({
                    next: async () => {
                        ...
                        const updates = await this.getUpdates(idvs, Long.fromNumber(0));
        ...
        if (ipBuf === undefined) {
            ...
            // Schedule update of this instance
